...
const StrategicMode: React.FC = () => {
  const [activeTab, setActiveTab] = useState<string>('configuration');
  const [selectedStrategy, setSelectedStrategy] = useState<Strategy | null>(null);
  const [selectedSpecialDate, setSelectedSpecialDate] = useState<Date | null>(null);
  const [isApplyingStrategy, setIsApplyingStrategy] = useState<boolean>(false);
  ...
  const handleSpecialDateSelect = (specialDate: Date) => {
    setSelectedSpecialDate(specialDate);
  };

  const alertColumns = [
    { field: 'title', label: 'Título', sortable: true },
    { field: 'severity', label: 'Severidade', sortable: true, render: (value: string) => (
    ...
    { field: 'created_at', label: 'Data', sortable: true, render: (value: string) => 
      new Date(value).toLocaleDateString('pt-BR')
    },
    { field: 'is_resolved', label: 'Status', sortable: true, render: (value: boolean) => (
  ...
  const actionColumns = [
    ...
    { field: 'status', label: 'Status', sortable: true, render: (value: string) => (
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">
                  Aplicada em {dashboardData.currentStrategy.applied_at ? new Date(dashboardData.currentStrategy.applied_at).toLocaleDateString('pt-BR') : ''}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <DataTable
              ...
              actions={[
                {
                  label: 'Resolver',
                  onClick: (item: Alert) => console.log('Resolver alerta:', item),
            ...
            <DataTable
              ...
              actions={[
                {
                  label: 'Detalhes',
                  onClick: (item: Action) => console.log('Ver detalhes:', item),

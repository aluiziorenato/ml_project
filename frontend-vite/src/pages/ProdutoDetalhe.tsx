...
const ProdutoDetalhe: React.FC = () => {
  ...
  const [metrics7d, setMetrics7d] = useState<Metrics | null>(null);
  const [metrics30d, setMetrics30d] = useState<Metrics | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [err, setErr] = useState<any>(null);

  const vendas30dSeries: Serie[] = [
  ...
  const conversao30dSeries: Serie[] = [
  ...
  useEffect(() => {
    ...
    const mock7d: Metrics = {
      sales: 28,
      revenue: 4321.5,
      visits: 2100,
      conversion: 1.4,
    };
    const mock30d: Metrics = {
      sales: 128,
      revenue: 17234.9,
      visits: 8421,
      conversion: 1.52,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      ...
      >
        <div>
          ...
          <p className="text-gray-600">
            ID: <span className="font-mono">{produto.id}</span> • SKU: <span className="font-mono">{produto.sku}</span>
          </p>
        </div>
      ...
      >
        ...
        <KPICard
          title="Receita 7d"
          value={metrics7d?.revenue != null ? metrics7d.revenue.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "-"}
          change="+6%"
          changeType="positive"
          icon="💵"
          color="purple"
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm border">
          ...
          <div>
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${produto.status === "Ativo" ? "bg-green-100 text-green-800" : produto.status === "Baixo Estoque" ? "bg-orange-100 text-orange-800" : "bg-red-100 text-red-800"}`}
            >
              {produto.status}
            </span>
          </div>
      ...
      >
        <div className="rounded-xl bg-white p-4 shadow-sm border">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Vendas diárias (30 dias)</h3>
          <GraficoVendasDiarias data={vendas30dSeries} />
        </div>
        <div className="rounded-xl bg-white p-4 shadow-sm border">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Visitas x Conversão</h3>
          <GraficoVisitasConversao visitas={visitas30dSeries} conversao={conversao30dSeries} />
        </div>
      </motion.div>
    </div>
  );
